...
$sql = "SELECT * FROM tb_users WHERE (user_fname LIKE '$name%' OR user_id  LIKE '$name%' OR user_lname LIKE '$name%') AND (role_id != 1)";
...
if ($query) {
    while ($row = mysqli_fetch_assoc($query)) {
        $data .= '<tr>
                    <td>' . $row['user_id'] . '</td>
                    <td><img src="img/' . $row["user_img"] . '" width="90" height="90" alt=""></td>
